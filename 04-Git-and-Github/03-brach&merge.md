...
<br>
`-d` merupakan option untuk menghapus untracked direktori dan `-f ` merupakan required option untuk menjalankan penghapusan (singkatan dari force), jika tidak ditambahkan `-f` maka git tidak akan melakukan penghapusan kecuali konfigurasi option clean.requireForce diset menjadi false. Kombinasi `-d` dan `-f` akan memberitahu kepada git clean untuk menghapus untracked directory.
